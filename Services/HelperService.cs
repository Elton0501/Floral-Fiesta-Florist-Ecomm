...
namespace Services
{
    public class HelperService
    {
        ...
        public void AddStarRating(int pId, int starRating)
        {
            using (var context = new SVIContext())
            {
                var data = context.Products.FirstOrDefault(x => x.Id == pId);
                var newRating = (Convert.ToInt32(data.Rating.HasValue ? data.Rating : 3) + starRating) / 2;
